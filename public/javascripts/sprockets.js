...
var Tipper = {
  tipify: function(element) {
    element.tipified = true;

    var title = element.title.replace(/\s+\[.*\]/, "").escapeHTML();
    if(title.blank()) return;

    element.writeAttribute("rel", title);
    element.writeAttribute("title", "");

    new Tip(element, element.readAttribute("rel"), {
      title: false,
      border: 2,
      borderColor: '#ccc',
      className: 'sortfolio',
  ...
  set_image_tips: function() {
    Event.addBehavior({
      'img.tipify_image:mouseover' : function(event) {
        var image = event.element();
        if(!image.tipified) Tipper.tipify(image);
      }
    });
  }
};
...
var FormFocus = {
  set_all: function() {
    ['listing_overview'].each(function(e) {
      FormFocus.focus(e);
    });
  },
...
Tipper.set_image_tips();

var Redirector = {
  redirect_to: function(path, time) {
    setTimeout(function() {
  ...
  set_click_behaviour: function() {
    Event.addBehavior({
      'a.homepage:click' : function(event) {
        ['location_id', 'budget_id', 'state_id', 'country_id'].each(function(c) { Cookie.remove(c)});
      }
    });
  }
};
...
document.observe("dom:loaded", function() {
  ...
  $$('.lazyimage').each(function(image) {
    var lazy = new ImageLoader(image);
    lazy.load();
  });
});
document.observe("dom:loaded", function() {
  ...
  var threshold    = $("header").getHeight();
  var height       = element.getHeight();
  var visible      = false;
  var hiddenStyle  = "bottom: -" + height + "px; opacity: 0";
  var visibleStyle = "bottom: 0; opacity: 1";
  var effect;

  element.setStyle(hiddenStyle);

  function show() {
    if (visible) return;
    animateTo(visibleStyle);
    visible = true;
  }

  function hide() {
    if (!visible) return;
    animateTo(hiddenStyle);
    visible = false;
  }

  function animateTo(style) {
    if (effect) effect.cancel();
    effect = new Effect.Morph(element, {
      style:       style,
      duration:    0.3,
...
var Braintree = {
  ...
  TransparentRedirectForm: {
    validationCallback: function(name, failed) {},

    prepare: function(event) {
      this.getInputs('submit').invoke('disable');

      if (Braintree.TransparentRedirectForm.validate(this)) {
        Braintree.TransparentRedirectForm.setExpiresOn(this);
        Braintree.TransparentRedirectForm.setZipCode(this);
        Braintree.TransparentRedirectForm.setRedirect(this);
        Braintree.TransparentRedirectForm.successfulSignup();
      } else {
        event.stop();
        this.getInputs('submit').invoke('enable');
      }
    },
    ...
    validate: function(form) {
      ...
      var callback = Braintree.TransparentRedirectForm.validationCallback;

      if (!Braintree.CreditCard.isValid($F('card_number'))) {
        callback('card_number', true);
        errors.push("Please enter a valid credit card number");
      } else {
        callback('card_number', false);
      }

      if ($('card_verification_value')) {
        if(!$F('card_verification_value').match(/^\d{3,4}$/)) {
          callback('card_verification_value', true);
          errors.push("Please enter a valid verification value");
        } else {
          callback('card_verification_value', false);
        }
      }
    ...
    serialize: function(form) {
      return Form.serializeElements(form.getElements().reject(function(e) {
        return e.className.match(/transaction_field/) || e.type == 'submit';
      }));
    },

    setExpiresOn: function(form) {
      var month = $('card_expires_on_month');
      var year  = $('card_expires_on_year');

      if (month && year) {
        $('card_expires_on').value = $F(month) + '/' + $F(year).substr(2);
      }
    },

    setZipCode: function() {
      if ($('card_zip_code') && $F('card_zip_code').blank()) {
        $('card_zip_code').value = '000000';
      }
    },

    setRedirect: function(form) {
      $('redirect').value += '&' + Braintree.TransparentRedirectForm.serialize(form);
    }
  }
};

document.observe('dom:loaded', function() {
  var form = $('transparent_redirect_form');
  if (form) form.observe('submit', Braintree.TransparentRedirectForm.prepare);
});
var Cities = {
  jump_to_country: function(event) {
    if(!$F('countries_jump_down').blank())
    {
      window.location.hash = $F('countries_jump_down');
    }
  }
};

document.observe("dom:loaded", function() {
  if(!$('countries_jump_down')) return;
  $('countries_jump_down').observe('change', Cities.jump_to_country);
});
var EndlessPage = {
  pro_start: null,
  pro_offset: null,
  ...
  check_scroll: function() {
    if (EndlessPage.near_bottom()) {
      var params = {pro_start: EndlessPage.pro_start, pro_offset: EndlessPage.pro_offset, free_start: EndlessPage.free_start, free_offset: EndlessPage.free_offset};

      $('live_spinner').show();

      var location;
      if(document.location.pathname.blank() || document.location.pathname == '/') {
        location = document.location + 'listings.js'
      } else {
        location = document.location + '.js';
      }

      new Ajax.Updater('endless_listings', location, {method:'get', parameters: params, insertion: 'bottom',
        evalScripts: true,
        onSuccess: function(transport) {
          $('live_spinner').hide();
        }
  ...
  near_bottom: function() {
    return EndlessPage.scroll_distance_from_bottom() < 100;
  },
...
var HaySession = {
  populate: function(cookie) {
    HaySession.session_data = cookie ? Base64.decode(cookie.sub(/\-\-\w+/, '')) : '';
  },

  has: function(key) {
    return(HaySession.session_data.include(key));
  },

  is_logged_in: function() {
    return(HaySession.has('user_id'));
  },

  adjust_links: function() {
    if(HaySession.is_logged_in()) {
      HaySession.set_visibility('hide', 'show');
    } else {
      HaySession.set_visibility('show', 'hide');
    }
  },

  set_visibility: function(guest, logged_in) {
    $$('.guest_user').invoke(guest);
    $$('.logged_in_user').invoke(logged_in);
  }
};

HaySession.populate(Cookie.get('_sortfolio_session'));

document.observe("dom:loaded", function() {
  HaySession.adjust_links();
});

var Favorites = Class.create({
  initialize: function(wrapper) {
    this.wrapper = wrapper;
    this.listing_id = this.wrapper.readAttribute('data-listing-id');

    if($('favorites')) {
      this.wrapper.ancestors().each(function(e) {
        if(e.classNames().include('listing_content') && !this.listing) this.listing = e;
      }.bind(this));
    }

    this.add_link = this.wrapper.getElementsBySelector('.add_to_favorites')[0];
    this.remove_link = this.wrapper.getElementsBySelector('.remove_from_favorites')[0];

    if(Favorites.favorite_ids.include(this.listing_id)) {
      this.show_remove_link();
    } else {
      this.show_add_link();
    }
  ...
  add_observers: function() {
    this.add_link.observe('click', this.favorite_added.bind(this));
    this.remove_link.observe('click', this.favorite_removed.bind(this));
  },

  favorite_added: function(transport) {
    new Ajax.Request(this.add_link.href, {method: 'get', onSuccess: function() {
      Notify.added('Added to favorites');
      this.show_remove_link();
      this.update_counters(1);
    }.bind(this)});
  },

  favorite_removed: function(transport) {
    new Ajax.Request(this.remove_link.href, {method: 'get', onSuccess: function() {
      Notify.removed('Removed from favorites');
      this.show_add_link();
      this.update_counters(-1);

      if(this.listing) this.listing.remove();
    }.bind(this)});
  },

  update_counters: function(offset) {
    $$('.favorites_count').each(function(e) {
      var current = e.innerHTML;
      current = current.sub(/\d+/, function(match){return parseInt(match[0]) + offset});
      e.update(current);
    });
  },

  show_add_link: function() {
    this.remove_link.hide();
    this.add_link.show();
  },

  show_remove_link: function() {
    this.add_link.hide();
    this.remove_link.show();
  }
});

Favorites.set_favorites_ids = function(ids) {
  Favorites.favorite_ids = ids;
};

Favorites.get_favorites_ids = function() {
  if(Cookie.get("favorites")) {
    new Ajax.Request('/favorites.js', {
      method:'get',
      onSuccess: function(transport){
        Favorites.favorite_ids = transport.responseText.evalJSON();
        Favorites.set_counters(Favorites.favorite_ids.size());
      }
    });
  } else {
    Favorites.favorite_ids = [];
    Favorites.set_counters(0);
    ajaxify_favorite_links();
  }
};

Favorites.set_counters = function(size) {
  $$('.favorites_count').each(function(e) {
    e.update('(' + size + ')');
  });
};

function ajaxify_favorite_links() {
  if(!Favorites.favorite_ids) return;

  $$('.favorite_links').each(function(fav_span) {
    if(!fav_span.processed_favorite) {
      new Favorites(fav_span);
      fav_span.processed_favorite = true;
    }
  });
};

document.observe("dom:loaded", function() {
  if(!Favorites.favorite_ids) {
    Favorites.get_favorites_ids();
  } else {
    Favorites.set_counters(Favorites.favorite_ids.size());
    ajaxify_favorite_links();
  }
...
Ajax.Responders.register({
  onComplete: function() {
    setTimeout(function() { ajaxify_favorite_links() }, 10);
  }
});
...
document.observe("dom:loaded", function() {
  ...
  var element = $("slide_listed");
  element.show();
  var height = element.getHeight();
  element.setStyle("height: 0");
  var animation;

  function animateToHeight(height) {
    if (animation) animation.cancel();
    animation = new Effect.Morph(element, {
      duration: 0.25,
      style: "height: " + height + "px;",
      afterFinish: function() { animation = false }
    });
  }

  $("signup").observe("mouseenter", function() {
...
Object.extend(Object.extend(Glider.prototype, Abstract.prototype), {
  initialize: function(wrapper, options){
    ...
    this.lazy_image_loaders = [];
    if(this.options.lazy_images) {
      var lazy_images = this.wrapper.getElementsBySelector('.slideshow_image');
      lazy_images.splice(0, 1);

      this.lazy_image_loaders = lazy_images.map(function(i) {
        return(new ImageLoader(i, {height: '300', width: '430', show_spinner: false, spinner_image: "/images/spinner-small.gif"}));
      });
    }
  ...
  click: function(event) {
    ...
    this.make_active_control(element);
  ...
  moveTo: function(element, container, options){
    ...
    this.make_active_control(this.controls[this.current._index]);
    ...
    if(this.lazy_image_loaders.any() && !this.loading_lazy_images) {
      this.loading_lazy_images = true;
      this.lazy_image_loaders.each(function(lazyimg) { lazyimg.load() });
      this.lazy_image_loaders = [];
    }
  ...
  make_active_control: function(element) {
    this.wrapper.getElementsBySelector('div.controls a.active').invoke('removeClassName', 'active');
...
var SlideShow = {
  init_all: function() {
    $$('.slideshow').each(function(slide) { SlideShow.create_glider(slide) })
  },

  create_glider: function(slide) {
    if (!slide.initialized_glider) {
      var glider = new Glider(slide.identify(), {duration:0.5, lazy_images: true});

      var links = slide.getElementsBySelector('ul li a');

      if (links.any()) {
        glider.make_active_control(links[0]);

        var control = slide.getElementsBySelector('.controls')[0];
        var margin = (100 + ((6 - links.size()) * 6)) + 'px';
        control.setStyle({marginLeft: margin});
      }

      slide.getElementsBySelector('.next').each(function(link) {
        link.observe('click', function() { glider.next() });
      });

      slide.getElementsBySelector('.previous').each(function(link) {
        link.observe('click', function() { glider.previous() });
      });

      slide.initialized_glider = true;
    }
  }
};

document.observe("dom:loaded", function() { SlideShow.init_all() });

Ajax.Responders.register({
  onComplete : function() {
    setTimeout(function() { SlideShow.init_all() }, 10);
  }
});
...
Gr0wl.Base = Class.create({
  ...
  create: function(styles) {
    ...
    this.block.setStyle('position:absolute;display:none;z-index:999;color:#444;font: 12px/14px "Lucida Grande", Arial, Helvetica, Verdana, sans-serif;'+styles.div);
  ...
  show: function(options) {
    ...
    elements.each(function(e, i) {
      ...
      if(i) {
        ...
        img.next().update(options.title).next();
      }
    });
    ...
    new Effect.Event({
      beforeStart: function() {
        new Effect.Parallel([
          new Effect.Opacity(elements[0], Object.extend(fxOpts, {to: 1})),
          new Effect.Opacity(elements[1], Object.extend(fxOpts, {to: 1}))
  ...
  hide: function(elements) {
    ...
    new Effect.Parallel([
      new Effect.Opacity(elements[0], Object.extend(fxOpts, {from: 1})),
      new Effect.Opacity(elements[1], Object.extend(fxOpts, {from: 1}))
    ], {
      duration:0.8,
      queue:'end',
      afterFinish:(function() {
...
Gr0wl.Smoke = Class.create(Gr0wl.Base, {
  ...
  create: function($super, oArgs) {
    $super({
      ...
      img: 'float:left;margin:20px 5px 0 20px;',
      h3: 'margin:0;margin-top:28px;font-size:16px;',
...
var ImageLoader = Class.create({
  initialize: function(image, options) {
    ...
    this.spinner_image = this.options.spinner_image || "/images/white_spinner.gif",

    this.make_lazy();
  },

  make_lazy: function() {
    this.image.original_src = this.image.src;

    this.image.src = this.blank_image;
    this.image.setStyle({ backgroundImage: 'url(' + this.spinner_image + ')', backgroundPosition: '50% 50%', backgroundRepeat: 'no-repeat' });
    if(this.options.height) this.image.height = this.options.height;
    if(this.options.width) this.image.width = this.options.width;
  },

  new_image_loaded: function() {
    this.image.replace(this.new_image);
    this.image.writeAttribute({height: false});
    if(this.options.width) this.image.writeAttribute({width: false});
  },

  load: function() {
    this.new_image = this.image.clone();
    this.new_image.src = this.image.original_src;

    if (this.new_image.complete) {
      this.new_image_loaded();
    } else {
     this.new_image.onload = this.new_image_loaded.bind(this);
    }
  }
});
var ListingForm = Class.create({
  initialize: function(form_element) {
    this.form_element = form_element;
    this.location_id = $('listing_location_id');
    this.other_location = $('listing_other_location');

    this.location_id.observe('change', this.location_id_selected.bind(this));
    this.location_id_selected();

    $('listing_in_us_yes').observe('change', this.in_us_changed.bind(this));
    $('listing_in_us_no').observe('change', this.in_us_changed.bind(this));
    this.in_us_changed();
  },

  location_id_selected: function() {
    var location = this.location_id.options[this.location_id.selectedIndex].innerHTML;

    if(location == 'My city is not listed') {
      $('other_listing_location').appear({duration: 0.3});
    } else {
      $('other_listing_location').fade({duration: 0.3});
    }
  },

  in_us_changed: function() {
    var in_us = this.form_element.getInputs('radio', 'listing[in_us]').find(function(radio) { return radio.checked; }).value;

    if (in_us == 'no') {
      $('other_listing_location_country').show();
      $('other_listing_location_state').hide();
    } else {
      $('other_listing_location_state').show();
      $('other_listing_location_country').hide();
    }
  }
});

document.observe("dom:loaded", function() {
  $$('.listing_form').each(function (form){
    new ListingForm(form);
  });
});
var Notify = {
  added: function(message) {
    Growl.Smoke({title: message, duration: 2.0, image: '/images/icon_check-large.png'});
  },

  removed: function(message) {
    Growl.Smoke({title: message, duration: 2.0, image: '/images/icon_x-large.png'});
  }
};
var PortfolioImageUpload = Class.create({
  initialize: function(portoflio_form_id) {
    this.portoflio_form_id = portoflio_form_id;
    this.pi_form = $(portoflio_form_id);
    this.portfolio_image_spinner = this.pi_form.getElementsBySelector('div.portfolio_spinner')[0];
    this.portfolio_image = this.pi_form.getElementsBySelector('div.current_image')[0];
    this.description_spinner = this.pi_form.getElementsBySelector('div.description_spinner')[0];

    this.description_input = this.pi_form.getElementsBySelector('div.description_input')[0];
    this.description_content = this.pi_form.getElementsBySelector('div.description_content')[0];
    this.pi_form.getElementsBySelector('.edit_description').invoke('observe', 'click', this.show_description_form.bind(this));

    this.pi_form.getElementsBySelector('.save_description').invoke('observe', 'click', this.submit_form.bind(this));
    this.pi_form.getElementsBySelector('.portfolio_image_file').invoke('observe', 'change', this.process_file_input.bind(this));
  },

  submit_form: function() {
    this.description_spinner.show();
  },

  show_description_form: function() {
    this.pi_form.getElementsBySelector('.edit_description').invoke('toggle');

    Effect.toggle(this.description_input, 'slide', { duration: 0.1 });
    this.description_content.toggle();
  },

  process_file_input: function(event) {
    this.portfolio_image.hide();
    this.portfolio_image_spinner.show();
...
var SearchBar = {
  click_location_menu: function(event) {
    Event.stop(event);

    $('search_budget_menu').hide();
    $('search_location_menu').toggle();
  },

  click_budget_menu: function(event) {
    Event.stop(event);

    $('search_location_menu').hide();
    $('search_budget_menu').toggle();
  },

  hide_all_menus: function() {
    $$('.search_menu').each(function(e) {e.hide();})
  },

  init: function(locations, budgets, location_url, budget_url) {
    $('search_location_name').observe('click', SearchBar.click_location_menu);
    $('search_budget_name').observe('click', SearchBar.click_budget_menu);

    document.observe('click', SearchBar.hide_all_menus);
  }
};

document.observe('dom:loaded', function() {
  if(!$('search_location_menu')) return;

  SearchBar.init();
});
var LiveImagePreview = {
  upload_file_id: 'upload_progress_file',
  upload_progress_bar_id: 'upload_progress_bar',

  activate_ajaxy_uploads: function(form_id, preview_action, preview_target) {
    LiveImagePreview.form_id = form_id;
    LiveImagePreview.preview_action = preview_action;
    LiveImagePreview.preview_target = preview_target;

    LiveImagePreview.original_action = $(LiveImagePreview.form_id).action;

    $('listing_portfolio_images_attributes_0_image').observe('change', LiveImagePreview.process_file_input);
  },

  process_file_input: function() {
    $(LiveImagePreview.upload_file_id).hide();
    $(LiveImagePreview.upload_progress_bar_id).show();

    $(LiveImagePreview.form_id).target = LiveImagePreview.preview_target;
    $(LiveImagePreview.form_id).action = LiveImagePreview.preview_action;

    $(LiveImagePreview.form_id).submit();

    $(LiveImagePreview.form_id).target = '';
    $(LiveImagePreview.form_id).action = LiveImagePreview.original_action;
  },

  preview_upload_complete: function(image_url) {
    $(LiveImagePreview.upload_file_id).show();
    $(LiveImagePreview.upload_progress_bar_id).hide();

    $('preview_card_image').src = image_url;
  }
};

var ListingSignup = {
  show_preview_company_name: function() {
    var name = $F('listing_company_name');
  ...
  show_preview_budget: function() {
    var checked = $('signup_listing_form').getInputs('radio', 'listing[budget_id]').find(function(radio) { return radio.checked; });

    if(checked) {
      var value = $$('label[for="' + checked.identify() + '"] > span.budget_label')[0].innerHTML;
      $('live_budget').update(value);
    }
  },
  ...
  show_preview_location: function() {
    ...
    {
      var location = $('listing_location_id').options[$('listing_location_id').selectedIndex].innerHTML;

      if(location == 'My city is not listed') {
        if($F('listing_other_location').blank()) {
          new_location = default_location;
        } else {
          new_location = $F('listing_other_location');
        }
      } else {
    ...
    $('live_company_location').update(new_location);
  },

  show_preview_other_location: function() {
    if(!$('live_company_location')) return;
    $('live_company_location').update($F('listing_other_location'));
  },
  ...
  activate_live_preview: function() {
    $('listing_company_name').observe('onkeydown', ListingSignup.show_preview_company_name);
    $('listing_company_name').observe('keyup', ListingSignup.show_preview_company_name);
    $('listing_company_name').observe('DOMFocusOut', ListingSignup.show_preview_company_name);

    $('listing_other_location').observe('onkeydown', ListingSignup.show_preview_other_location);
    $('listing_other_location').observe('keyup', ListingSignup.show_preview_other_location);
    $('listing_other_location').observe('DOMFocusOut', ListingSignup.show_preview_other_location);

    $('signup_listing_form').getInputs('radio', 'listing[budget_id]').each(function(button) {
      button.observe('change', ListingSignup.show_preview_budget);
    });

    $('listing_location_id').observe('change', ListingSignup.show_preview_location);

    $('listing_portfolio_images_attributes_0_description').observe('change', ListingSignup.show_preview_image_description);
    $('listing_portfolio_images_attributes_0_description').observe('keyup', ListingSignup.show_preview_image_description);
  },

  set_fields: function() {
    ListingSignup.show_preview_company_name();
    ListingSignup.show_preview_other_location();
    ListingSignup.show_preview_budget();
    ListingSignup.show_preview_location();
    ListingSignup.show_preview_image_description();
  },

  install_form_validators: function() {
    var form = $('queenbee_redirect_form');

    if(form) {
      form.observe('submit', ListingSignup.validate_without_braintree);
    } else {
      form = $('transparent_redirect_form');

      ListingSignup.original_validator = Braintree.TransparentRedirectForm.validate;
      Braintree.TransparentRedirectForm.validate = ListingSignup.validate_form;
      ListingSignup.original_validation_callback = Braintree.TransparentRedirectForm.validationCallback;
      Braintree.TransparentRedirectForm.validationCallback = ListingSignup.validation_callback;
    }

    $('qb_data_owner_email').observe('change', ListingSignup.validate_email_uniqueness)
    $('qb_data_password_confirmation').observe('change', ListingSignup.validate_password_confirmation)
  },

  validate_without_braintree: function(event) {
    if(!ListingSignup.validate_form(this)) {
      event.stop();
    }
  },
  ...
  validate_form: function(form) {
    ...
    ListingSignup.validate_email(form, errors);
    ListingSignup.validate_password(form, errors);
    ...
    if(ListingSignup.original_validator)
      return ListingSignup.original_validator(form);

    return true;
  },

  validation_callback: function(field, failed) {
  },

  email_regex: /^[\w\.%\+\-]+@(?:[A-z0-9\-]+\.)+(?:[A-Z]{2}|com|org|net|edu|gov|mil|biz|info|mobi|name|aero|jobs|museum)$/i,

  validate_email: function(form, errors) {
    var email = $('qb_data_owner_email');

    if(!email.present()) {
      errors.push("You must provide your email address.");
    } else if(!ListingSignup.email_regex.match(email.value)) {
      errors.push("Email address must look like an email address.");
    }
  },

  validate_password: function(form, errors) {
    var password = $('qb_data_password');
    var confirmation = $('qb_data_password_confirmation');

    if(!password.present()) {
      errors.push('You must choose a password.');
    } else if(password.value.length < 6) {
      errors.push('You must choose a password at least 6 characters long.');
    } else if(confirmation.value != password.value) {
      errors.push("Your password confirmation doesn't match your password.");
    }
  },

  validate_email_uniqueness: function() {
    new Ajax.Request('/signup/validate', {
      asynchronous:true,
      evalScripts:true,
      method:'get',
      parameters:'owner_email=' + escape($F('qb_data_owner_email'))});
  },

  validate_password_confirmation: function() {
    var password = $F('qb_data_password');
    var confirmation = $F('qb_data_password_confirmation');
...
document.observe("dom:loaded", function() {
  if(!$('signup_listing_form')) return;

  ListingSignup.activate_live_preview();
  ListingSignup.set_fields();
  LiveImagePreview.activate_ajaxy_uploads('signup_listing_form', '/signup/image_preview', 'upload_iframe');
});
var UserForm = {
  validate_password: function() {
    var password = $F('user_password');
    var password_confirmation = $F('user_password_confirmation');
  ...
  validate_username: function() {
    new Ajax.Request('/account/check_email', { asynchronous:true, evalScripts:true, method:'get', parameters:'email=' + escape($F('user_email'))});
  },

  observe_form_fields: function() {
    var user_email = $('user_email')
    if(user_email) user_email.observe('change', UserForm.validate_username);

    var user_password_confirmation = $('user_password_confirmation')
    if(user_password_confirmation) user_password_confirmation.observe('change', UserForm.validate_password);
  },

  validate_form: function() {
    if($$('.account_form').any()) {
      UserForm.observe_form_fields();
    }
  }
};

document.observe('dom:loaded', UserForm.validate_form);
